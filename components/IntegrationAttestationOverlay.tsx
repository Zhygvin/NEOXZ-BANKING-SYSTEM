...
const IntegrationAttestationOverlay: React.FC<IntegrationAttestationOverlayProps> = ({ onComplete, merchantId }) => {
  const [steps, setSteps] = useState([
    { id: 'SDS_CORE', label: 'NEOXZ Sovereign Core v16.2', status: 'PENDING' },
    { id: 'MTLS_HANDSHAKE', label: 'Mutual TLS Hardware Pinning', status: 'PENDING' },
    { id: 'EXTERNAL_BLOCK', label: 'External Rail Lockdown (G-Pay Blocked)', status: 'PENDING' },
    { id: 'PARITY_SYD', label: 'Consortium Grounding (SDS_ANCHOR)', status: 'PENDING' },
    { id: 'PCI_SOV', label: 'Sovereign-Grade Audit Compliance', status: 'PENDING' },
    { id: 'MANDATE_LOCK', label: 'Irrevocable Beneficiary Alignment', status: 'PENDING' }
  ]);
  ...
  return (
    <div className="fixed inset-0 z-[12000] bg-black/98 backdrop-blur-3xl flex items-center justify-center p-8 animate-in fade-in duration-500 font-sans">
      ...
      <div className="w-full max-w-3xl bg-[#0a0a0a] border-[3px] border-emerald-500/30 rounded-[3rem] p-12 shadow-[0_0_150px_rgba(16,185,129,0.2)] relative overflow-hidden flex flex-col items-center">
        
        <header className="flex flex-col items-center gap-6 mb-10 relative z-10">
           <div className="p-6 rounded-full bg-emerald-600 text-black shadow-2xl shadow-emerald-500/30 border-4 border-emerald-400">
              <ShieldCheck className="w-10 h-10 animate-pulse" />
           </div>
           <div className="text-center">
              <h2 className="text-3xl font-black text-white uppercase tracking-tighter mb-2">Sovereign API Attestation</h2>
        ...
        <div className="w-full mt-10 relative z-10">
           {!isSubmitting ? (
             <div className="flex items-center justify-center gap-4 text-emerald-500/80">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest">SECURE_BYPASS_ENGAGED</span>
             </div>
           ) : complete ? (
             <div className="w-full py-5 rounded-3xl bg-emerald-500 text-black font-black uppercase tracking-[0.3em] text-xs flex items-center justify-center gap-4 shadow-2xl shadow-emerald-500/20 animate-in zoom-in">
                ...
                SOVEREIGN ATTESTATION SUCCESSFUL
             </div>
           ) : (
             <div className="w-full py-5 rounded-3xl bg-emerald-600 text-white font-black uppercase tracking-[0.3em] text-xs flex items-center justify-center gap-4 shadow-2xl shadow-emerald-500/20">
