...
import { 
  ...
  Code, Navigation, Hash, ShieldX, FileBadge
} from 'lucide-react';
...
const NEOXZBankCommandCenter: React.FC<NEOXZBankCommandCenterProps> = ({ stats, onDirectiveExecute }) => {
  ...
  const isMerchantCalibrated = localStorage.getItem('neoxz_merchant_calibrated') === 'true';
         ...
         <div className="xl:col-span-4 p-10 rounded-[3.5rem] bg-slate-900/40 border border-amber-500/20 shadow-3xl flex flex-col justify-between relative overflow-hidden group">
            ...
            <div className="space-y-4 relative z-10">
               <div className="flex items-center justify-between">
                  <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-none">Merchant ID status</span>
                  <div className={`w-2 h-2 rounded-full ${isMerchantCalibrated ? 'bg-emerald-500 animate-pulse shadow-[0_0_8px_rgba(16,185,129,1)]' : 'bg-amber-500 animate-ping'}`}></div>
               </div>
               <div className={`text-4xl font-black transition-colors duration-700 mono tracking-tighter ${isMerchantCalibrated ? 'text-emerald-400 glow-emerald' : 'text-amber-400'}`}>
                  {isMerchantCalibrated ? 'LOCKED_0x7F' : 'CALIBRATING...'}
               </div>
               <p className="text-[9px] text-slate-400 font-medium uppercase tracking-widest leading-relaxed">
                 {isMerchantCalibrated ? 'BCR2DN4TU7BMDMDU is anchored to Google Cloud Developer neoxz-systems-v16.' : 'Identity handshake pending for neoxz systems merchant core.'}
               </p>
            </div>
            <div className="pt-6 border-t border-white/5 relative z-10 flex justify-between items-center">
               <span className={`text-[9px] font-black uppercase tracking-widest ${isMerchantCalibrated ? 'text-emerald-500' : 'text-amber-500'}`}>
                  {isMerchantCalibrated ? 'SDS_IDENTITY_OK' : 'MANDATE_PENDING'}
               </span>
               <span className="text-[10px] font-mono text-white">BCR2DN...DMDU</span>
            </div>
         </div>
      </div>
           ...
           <div className="flex-1 bg-black border border-slate-800 rounded-[3rem] p-8 flex flex-col gap-6 overflow-hidden shadow-2xl relative">
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 font-mono text-[10px] relative z-10 pr-2">
                 {[
                   ...
                   { t: "14:20:08", m: "MERCHANT_PROFILE: BCR2DN...DMDU Verified [OK]", c: "text-emerald-500 font-bold" },
                   ...
                   { t: "14:20:45", m: "BSP_PH Mandate alignment check: SUCCESS", c: "text-amber-400" },
                 ].map((log, i) => (
              ...
              <div className={`p-6 rounded-[2rem] border flex items-center justify-between relative z-10 shrink-0 ${isMerchantCalibrated ? 'bg-emerald-500/5 border-emerald-500/10' : 'bg-amber-500/5 border-amber-500/10'}`}>
                 <div className="flex items-center gap-4">
                    <FileBadge className={`w-6 h-6 ${isMerchantCalibrated ? 'text-emerald-500' : 'text-amber-500'}`} />
                    <div className="flex flex-col">
                       <span className="text-[10px] font-black text-white uppercase tracking-widest">Merchant Calibration</span>
                       <span className={`text-[8px] font-bold uppercase ${isMerchantCalibrated ? 'text-emerald-500' : 'text-amber-500'}`}>
                          {isMerchantCalibrated ? 'LOCKED_&_VERIFIED' : 'PENDING_UPGRADE'}
                       </span>
                    </div>
                 </div>
                 <CheckCircle2 className={`w-5 h-5 ${isMerchantCalibrated ? 'text-emerald-500' : 'text-slate-700'}`} />
              </div>
           </div>
        </div>
