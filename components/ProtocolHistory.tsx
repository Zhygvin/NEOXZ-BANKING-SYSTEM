...
import { 
  ...
  ScrollText, ScanSearch, SortAsc, SortDesc, ArrowUpDown
} from 'lucide-react';
import { HistoryItem, ExecutionStep, GroundingSource, MandatePriority } from '../types';

interface ProtocolHistoryProps {
  history: HistoryItem[];
}

const PRIORITY_ORDER: Record<MandatePriority, number> = {
  'SOVEREIGN': 4,
  'CRITICAL': 3,
  'ELEVATED': 2,
  'STANDARD': 1
...
const ProtocolHistory: React.FC<ProtocolHistoryProps> = ({ history }) => {
  ...
  const processedHistory = useMemo(() => {
    ...
    if (sortMode === 'PRIORITY') {
      ...
      // Default to chronological (assume ID or separate field for true chrono if timestamp is just string)
      result.sort((a, b) => b.timestamp.localeCompare(a.timestamp));
    }

    return result;
  }, [history, filter, sortMode]);
  ...
  const getPriorityConfig = (priority: MandatePriority) => {
    switch (priority) {
      case 'SOVEREIGN': return { color: 'text-purple-400', bg: 'bg-purple-500/10', border: 'border-purple-500/30' };
      case 'CRITICAL': return { color: 'text-rose-500', bg: 'bg-rose-500/10', border: 'border-rose-500/30' };
      case 'ELEVATED': return { color: 'text-amber-500', bg: 'bg-amber-500/10', border: 'border-amber-500/30' };
      case 'STANDARD': 
      default: return { color: 'text-slate-400', bg: 'bg-slate-800/20', border: 'border-slate-800' };
    }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-black/60 border border-slate-900 rounded-[4rem] overflow-hidden shadow-3xl backdrop-blur-2xl transition-all hover:border-emerald-500/20">
      <div className="p-12 border-b border-slate-900 bg-black/40 flex flex-col md:flex-row md:items-center justify-between gap-8">
        ...
        <div className="flex flex-wrap items-center gap-4 bg-black/60 p-2 rounded-2xl border border-slate-800">
           <div className="flex items-center gap-3 mr-4 pl-4 border-r border-slate-800">
              <ArrowUpDown className="w-4 h-4 text-slate-500" />
              <button 
                onClick={() => setSortMode(sortMode === 'CHRONO' ? 'PRIORITY' : 'CHRONO')}
                className="text-[10px] font-black uppercase tracking-widest text-emerald-500 hover:text-emerald-400 transition-colors"
              >
                Sort: {sortMode === 'CHRONO' ? 'Recent' : 'Priority'}
              </button>
           </div>
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar">
        {processedHistory.length === 0 ? (
          ...
          processedHistory.map((item) => {
            const pStyle = getPriorityConfig(item.priority);
            return (
                  ...
                  <div className="flex items-center gap-10">
                    ...
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center gap-4">
                        ...
                        <div className={`px-2 py-0.5 rounded border ${pStyle.bg} ${pStyle.color} ${pStyle.border} text-[8px] font-black tracking-widest uppercase`}>
                           {item.priority}
                        </div>
                      </div>
